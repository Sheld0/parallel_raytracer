...
void render(const std::vector<Sphere> &spheres)
{
    ...
    if ( rank == 0) {
      ...
      while (count_task<Total) {
        std::vector<float> ligne(W*3);
        MPI_Recv(ligne.data(),3*W,MPI_FLOAT,MPI_ANY_SOURCE,MPI_ANY_TAG,MPI_COMM_WORLD,&status);
        ...
        count_task+=1;
        for (int j = 0; j < W*3; j++) {
          im[3*status.MPI_TAG*W+j]=ligne[j];
        }
      }

      for (int i = 0; i < numtasks-1; i++) {
        std::vector<int> ligne(W*3);
        MPI_Recv(ligne.data(),W*3,MPI_FLOAT,MPI_ANY_SOURCE,MPI_ANY_TAG,MPI_COMM_WORLD,&status);
        /*std::cout << "ligne rescv" << status.MPI_TAG << std::endl;
        std::cout << "pros " << status.MPI_SOURCE << std::endl;
        std::cout << "count_task " << count_task << std::endl;*/
        for (int j = 0; j < W*3; j++) {
                 ...
                 if(count_task >= 0)
                 {
                    ...
                    //    #pragma omp parallel for
                       for (unsigned x = 0; x < W; ++x) {
                           ...
                           ligne[3*x]=image[W*y+x].x;
                           ligne[3*x+1]=image[W*y+x].y;
                           ligne[3*x+2]=image[W*y+x].z;
                           //std::cout << image[W*y+x].x << std::endl;
                       }

                       /*std::cout << "ligne env " << count_task << std::endl;
                       std::cout << "from " << rank << std::endl;*/
                       MPI_Send(ligne.data(), 3*W, MPI_FLOAT, 0, count_task, MPI_COMM_WORLD);
                 }
                 //std::cout << "ligne env " << count_task << std::endl;
           }
     }
